								...
								<div class="p-100">
									<form class="d-flex flex-column">
										<div>
											...
											<input type="text" name="name" class="form_input w-100" v-model="injuriesForm.name" placeholder="напр.: Ушиб">
										</div>
										<div>
											<label for="date">Дата</label>
											<date-picker :date="injuriesForm.date" :type="'day'"></date-picker>
										</div>
										<div>
											<label for="note">Примечание</label>
											<textarea name="note" class="form_input w-100" v-model="injuriesForm.note"></textarea>
										</div>
										<button type="button" v-if="!injuriesForm.itemId" v-on:click="addMedicalRecord" class="btn btn-primary mx-auto">Добавить</button>
...
export default {
	...
	methods: {
		editMedicalRecord: function(id){
			this.$http.get(this.endpoint + this.item.type + '/' + id).then((response) => {
				if(response.data.medicalRecord){
					this.injuriesForm.itemId = response.data.medicalRecord._id;
					this.injuriesForm.name = response.data.medicalRecord.name;
					this.injuriesForm.date.time = response.data.medicalRecord.date.time;
					this.injuriesForm.note = response.data.medicalRecord.note;
					this.showModal = true;
				}else{
		...
		saveMedicalRecord: function(){
			this.$http.post(this.endpoint + this.item.type + '/' + this.injuriesForm.itemId, this.injuriesForm).then((response) => {
				if(response.data.medicalRecord){
					this.medicalRecordsList = [];
					this.getMedicalRecords();
					this.injuriesForm.itemId = '';
					this.injuriesForm.name = '';
					this.injuriesForm.date.time = '';
					this.injuriesForm.note = '';
